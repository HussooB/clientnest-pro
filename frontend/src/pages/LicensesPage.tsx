...
function LicenseFormModal({ open, onClose, license, presetClientId }: { open: boolean; onClose: () => void; license: LicenseRow | null; presetClientId?: string }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={license ? "Edit license" : "Grant license"} sub="Module C · product assignment & expiry tracking"
      footer={<>
        ...
        <Button loading={mutation.isPending} onClick={handleSubmit((f) => mutation.mutate(f))}>{license ? "Save changes" : "Grant license"}</Button>
      </>}>
      <form className="grid grid-cols-2 gap-4" onSubmit={handleSubmit((f) => mutation.mutate(f))}>
        ...
        <Field label="Seats" required error={errors.seats?.message}><TextInput error={!!errors.seats} type="number" min={1} {...register("seats")} /></Field>
        <Field label="Start date" required error={errors.startDate?.message}><TextInput error={!!errors.startDate} type="date" {...register("startDate")} /></Field>
...
export default function LicensesPage() {
  ...
  useEffect(() => {
    if (params.get("new") === "1" || params.get("clientId") || params.get("expiring")) setParams({}, { replace: true });
  }, []);
  ...
  return (
    <div>
      <PageHeader title="Products & Licenses" desc="Module C — catalog assignments with expiry radar. Red ≤ 30 days, amber ≤ 60, yellow ≤ 90."
        actions={manage && <Button onClick={() => { setFormLicense(null); setFormOpen(true); }}><IconPlus width={15} height={15} /> Grant license</Button>} />
