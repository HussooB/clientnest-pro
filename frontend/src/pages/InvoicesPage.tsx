...
import type { ClientRow, InvoiceRow, ListResponse } from "../types";
...
const invoiceSchema = z.object({
  ...
  items: z.array(z.object({ description: z.string().min(2, "Required"), qty: z.coerce.number().min(1, "≥1"), unitPrice: z.coerce.number().min(0, "≥0") })).min(1, "Add at least one line item"),
});
...
function InvoiceFormModal({ open, onClose, invoice, presetClientId }: { open: boolean; onClose: () => void; invoice: InvoiceRow | null; presetClientId?: string }) {
  ...
  useEffect(() => {
    ...
    if (invoice) {
      ...
      reset({ clientId: presetClientId ?? "", issueDate: d.toISOString().slice(0, 10), dueDate: due.toISOString().slice(0, 10), status: "Sent", taxRatePct: 15, items: [{ description: "Professional services", qty: 1, unitPrice: 1000 }] });
    }
  }, [open, invoice, presetClientId, reset]);

  const subtotal = (watched.items ?? []).reduce((s: number, it: any) => s + (Number(it.qty) || 0) * (Number(it.unitPrice) || 0), 0);
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Client" required error={errors.clientId?.message}>
            <Select error={!!errors.clientId} {...register("clientId")}>
              ...
              {getArr<ClientRow>(clientsQ.data).map((c) => <option key={c.id} value={c.id}>{c.companyName}</option>)}
            </Select>
          </Field>
          <Field label="Status" required><Select {...register("status")}><option>Draft</option><option>Sent</option></Select></Field>
          <Field label="Issue date" required error={errors.issueDate?.message}><TextInput error={!!errors.issueDate} type="date" {...register("issueDate")} /></Field>
          <Field label="Due date" required error={errors.dueDate?.message}><TextInput error={!!errors.dueDate} type="date" {...register("dueDate")} /></Field>
        </div>
        <div>
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[13px] font-semibold">Line items <span className="text-rose-600">*</span></span>
            <Button type="button" variant="subtle" size="xs" onClick={() => append({ description: "", qty: 1, unitPrice: 0 })}><IconPlus width={12} height={12} /> Add line</Button>
          </div>
          {errors.items && !Array.isArray(errors.items) && <p className="mb-2 text-xs font-medium text-rose-700">{(errors.items as any).message}</p>}
          <div className="space-y-2">
            {fields.map((field, i) => (
              <div key={field.id} className="flex items-start gap-2">
                <div className="flex-1"><TextInput placeholder="Description" error={!!(errors.items as any)?.[i]?.description} {...register(`items.${i}.description`)} /></div>
                <TextInput type="number" min={1} className="w-20 text-right" placeholder="Qty" error={!!(errors.items as any)?.[i]?.qty} {...register(`items.${i}.qty`)} />
                <TextInput type="number" min={0} step="0.01" className="w-28 text-right" placeholder="Unit price" error={!!(errors.items as any)?.[i]?.unitPrice} {...register(`items.${i}.unitPrice`)} />
                <Button type="button" variant="ghost" size="sm" className="mt-0.5 hover:text-rose-700" disabled={fields.length === 1} onClick={() => remove(i)} aria-label="Remove line"><IconTrash width={14} height={14} /></Button>
              </div>
            ))}
          </div>
        </div>
...
export default function InvoicesPage() {
  ...
  const rows = getArr<InvoiceRow>(invoicesQ.data);
  const total = invoicesQ.data?.total ?? 0;

  return (
    <div>
      <PageHeader title="Invoices & Payments" desc="Module D — billing, tax, payments, credit notes and statements of account."
        actions={manage && <Button onClick={() => { setFormInvoice(null); setFormOpen(true); }}><IconPlus width={15} height={15} /> New invoice</Button>} />
      <Card pad={false} className="animate-fade-up">
        <div className="flex flex-wrap items-center gap-2.5 border-b border-line/70 px-4 py-3">
          ...
          <Select value={status} onChange={(e) => setStatus(e.target.value)} className="w-[160px]">
            <option value="">All statuses</option>{INVOICE_STATUSES.map((s) => <option key={s}>{s}</option>)}
          </Select>
          ...
          <Table minWidth="min-w-[1060px]" head={<><Th>Invoice</Th><Th>Client</Th><Th>Issued</Th><Th>Due</Th><Th className="text-right">Subtotal</Th><Th className="text-right">Tax</Th><Th className="text-right">Total</Th><Th className="text-right">Paid</Th><Th className="text-right">Balance</Th><Th>Status</Th><Th className="text-right">Actions</Th></>}>
            {rows.map((i) => (
              <tr key={i.id} className="group cursor-pointer transition-colors hover:bg-brand-50/40" onClick={() => navigate(`/invoices/${i.id}`)}>
                ...
                <Td className="text-right"><span className={`tnum font-mono font-bold ${i.balance > 0 && i.status === "Overdue" ? "text-rose-700" : ""}`}>{fmtMoney(i.balance)}</span></Td>
                ...
                <Td className="text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
                    ...
                    {manage && <Button variant="ghost" size="xs" title="Edit" onClick={() => { setFormInvoice(i); setFormOpen(true); }}><IconPencil width={13} height={13} /></Button>}
                    {can(user?.role, "invoices.delete") && <Button variant="ghost" size="xs" title="Void invoice" className="hover:text-rose-700" onClick={() => setDeleteTarget(i)}><IconTrash width={13} height={13} /></Button>}
                  </div>
                </Td>
              </tr>
            ))}
