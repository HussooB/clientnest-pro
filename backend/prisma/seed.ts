...
import { PrismaClient, Role, ClientStatus, LeadStage, LeadSource, LicenseType, InvoiceStatus, PaymentCategory, HostingCycle, ContactType } from "@prisma/client";
...
async function main(): Promise<void> {
  ...
  // 2. Get admin user for account ownership
  ...
  const invoices = await Promise.all([
    ...
    prisma.invoice.create({
      data: {
        ...
        clientId: clients[1].id,
        issueDate: today,
        dueDate: new Date(today.getTime() - 10 * 24 * 60 * 60 * 1000), // 10 days ago (overdue)
        subtotal: 3000,
        taxRatePct: 10,
        taxAmount: 300,
        totalAmount: 3300,
  ...
  console.log("\n🎉 Seed complete! Database ready with:");
  console.log("  • 4 Users (Admin, Finance, Support, Sales)");
  console.log("  • 3 Products");
  console.log("  • 3 Clients");
  console.log("  • 2 Contacts");
  console.log("  • 3 Leads");
  console.log("  • 2 Licenses (1 expiring in 30 days)");
  console.log("  • 2 Invoices (1 overdue)");
